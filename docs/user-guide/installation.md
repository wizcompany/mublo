...
설치 전에 아래 3개 디렉토리에 웹 서버 사용자의 읽기/쓰기 권한이 있어야 합니다.

```bash
# 웹 서버 사용자(www-data)에게 소유권 부여
chown -R www-data:www-data config/ storage/ public/storage/

# 디렉토리: 읽기/쓰기/실행, 파일: 읽기/쓰기
chmod -R 775 config/
chmod -R 775 storage/
...
| 디렉토리 | 퍼미션 | 용도 |
|----------|--------|------|
| `config/` | 775 (설치 후 444 권장) | 설치 시 설정 파일 자동 생성 |
| `storage/` | 775 | 캐시, 로그, 세션, 임시 파일 |
| `public/storage/` | 775 | 업로드된 파일 (웹 접근 가능) |

> **참고:** 설치가 완료되면 `config/` 디렉토리의 설정 파일은 읽기 전용(444)으로 변경하는 것을 권장합니다. 자세한 내용은 [설치 후 보안 조치](#설치-후-보안-조치)를 참고하세요.
...
# 웹 서버 사용자에게 소유권 및 쓰기 권한 부여
chown -R www-data:www-data config/ storage/ public/storage/
chmod -R 775 config/ storage/ public/storage/
```
