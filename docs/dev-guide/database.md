...
공개 저장소의 현재 초기 마이그레이션은 첫 공개 릴리스 기준 최종 스키마를 반영한 베이스라인입니다.
즉 Core, 기본 Package, 공개 Plugin의 초기 `CREATE` 파일에는 공개 시점의 최종 컬럼이 이미 포함되어 있습니다.
공개 이후 스키마 변경이 생기면 그때부터 후속 번호의 업데이트 마이그레이션을 추가합니다.
